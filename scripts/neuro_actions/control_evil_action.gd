...
func _init(window: ActionWindow, game_manager:GameManager):
	...
	user = _game_manager.get_entity_by_name("Evil")
...
func _get_description() -> String:
	return "Makes Evil do an action on a target.\n Shock: medium damage, removes target AP, halves AP regen for 2 turns.\n Harpoon Throw: big damage \n Katana slash: huge damage." 
...
func _validate_action(data: IncomingData, state: Dictionary) -> ExecutionResult:
	if not user:#this should never ever happen
		return ExecutionResult.failure("Ooops, Evil has just exploded, try a different character")
...
func _execute_action(state: Dictionary) -> void:
	print("executing evil control action")
